...
@overload
def lru_cache(maxsize: int | None = 128, typed: bool = False) -> Callable[[Callable[..., _T]], _lru_cache_wrapper[_T]]: no_effects()
@overload
...
def singledispatch(func: Callable[..., _T]) -> _SingleDispatchCallable[_T]: no_effects()
...
class cached_property(Generic[_T_co]):
    ...
    attrname: str | None
    def __init__(self, func: Callable[[Any], _T_co]) -> None: no_effects()
    @overload
...
def cache(user_function: Callable[..., _T], /) -> _lru_cache_wrapper[_T]: no_effects()
def _make_key(
    args: tuple[Hashable, ...],
